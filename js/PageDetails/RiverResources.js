...
async function BindTable(token, id, regionId, basinId) {
    ShowLoader();
    try {
        const response = await fetch(ResolveUrl("/GetAllRiverDataTable"), {
            method: "POST",
        ...
        const html = `<table class="table table-bordered" id="projectTable">
                            <thead>
                              <tr>
                                ...
                                <th scope="col">Water Resources Project</th>
        ...
        tbody.innerHTML = result.map((value, index) => `
              <tr>
                ...
                <td>${value.name}</td>
                <td><a href="${ResolveUrl("/Dam/?riverId=" + FrontValue(value.riverId))}">Visit Page</a></td>
              </tr>`).join('');
