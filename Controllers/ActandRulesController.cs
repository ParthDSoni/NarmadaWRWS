...
{
    public class ActandRulesController : Controller
    {
        ...
        public ActandRulesController(IHttpClientFactory _httpClientFactory, ICMSTemplateMasterService _objCMSTemplateMasterService, IEcitizenMasterService _objEcitizenMasterService)
        {
        ...
        public JsonResult BindActandRuleGrid(string? Title)
        {

            try
            {
                List<EcitizenModel> lstNewsList = objEcitizenMasterService.GetListFront(LanguageId).Where(x => x.EcitizenTypeId == "1").ToList();

                if (lstNewsList.Count() <= 0)
                {
                    lstNewsList = objEcitizenMasterService.GetListFront(LanguageId).Where(x => x.EcitizenTypeId == "1").ToList();
                }
                if (Title != null)
                {
                    lstNewsList = lstNewsList.Where(x => x.EcitizenTitle.ToLower().Contains(Title.ToLower())).ToList();
                }
